...
#define CACHELINE 512
	...
	int variance[class][numberOfRuns];
...
//				printf("%d -> %d\n", l / delta, (int) time2);
				...
				variance[l/delta][j] = (int) time2;
			}
		}

		//calcolo la varianza
		double mean, sd, var, dev, sum = 0.0, sdev = 0.0;
		...
		for (int i = 0; i < class; i++) {
			...
			sd = 0;
			for (int j = 0; j < numberOfRuns; j++) {
				sum = sum + variance[i][j];
				mean = sum / numberOfRuns;
				dev = (variance[i][j] - mean) * (variance[i][j] - mean);
				sdev = sdev + dev;
			}
			var = sdev / (class - 1);
			sd = sqrt(var);
